...
### Session 2: Harder Tasks (In Progress)

To address the ceiling effect, we're running a second experiment with a more complex task (analyzeUserActivity, 500+ points). Preliminary solo results show 525/550 (95.45%) — still strong but with room for differentiation. Unstructured and structured conditions are running now, with results expected by the end of today's session.
